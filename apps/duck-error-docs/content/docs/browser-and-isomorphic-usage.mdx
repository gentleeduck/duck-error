...
description: @gentleduck/errors has zero runtime dependencies and ships both ESM and CJS, so it runs in the browser, edge runtimes, and React Native — with one real gotcha on the client.
---

## Where it runs

`packages/duck-errors` has zero runtime dependencies and builds to both ESM (`dist/index.js`)
