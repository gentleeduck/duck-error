...
```typescript
function quickFail(code: AppError.Bare) {
  return throwAppError(code)
}

quickFail('NOT_FOUND')      // fine
quickFail('RATE_LIMITED')   // Error: 'RATE_LIMITED' is not assignable to Bare
```
...
```typescript
function isRetryable(err: AppError): err is AppError<AppError.Faults> {
  return (Object.keys(FAULT_CODES) as AppError.Faults[]).includes(err.code as AppError.Faults)
}
```
