---
title: API reference
description: Every export of @gentleduck/errors — createErrorKit, the brand helpers, and the secret-scrubbing utilities — with full signatures.
---
...
Pass the registry `as const satisfies Record<string, number>`. Both matter: `as const` is what
lets each code's status carry its brand (see below) into the returned type; `satisfies` checks
every value is a plain number without widening the literal types away.
...
generated straight from `packages/duck-errors/src` with TypeDoc.
</Callout>
