---
title: NestJS
description: Map @gentleduck/duck-error instances to NestJS HTTP responses with an ExceptionFilter, using statusCode for compatibility with Nest's base filter.
---
...
export class WidgetsService {
  async findOne(id: string) {
    const widget = await this.db.widgets.findById(id)
