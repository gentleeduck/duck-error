---
title: Next.js
description: Handle @gentleduck/duck-error instances in Route Handlers and Server Actions, the two places Next.js needs a manual catch.
---
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const widget = await db.widgets.findById(params.id)
    if (!widget) throwAppError('NOT_FOUND', { resource: 'widget', id: params.id })
