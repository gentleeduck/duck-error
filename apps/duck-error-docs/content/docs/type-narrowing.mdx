...
try {
  ...
  if (err instanceof AppError) {
    // err: AppError<AppError.Code> — any code in the registry, meta is Record<string, unknown>
  }
}
```

`instanceof AppError` tells you the error came from this kit. It does not narrow `err.code` to
a specific literal, so `err.meta` is still the union of every code's metadata shape --
effectively untyped. Narrowing to one code needs a second check.
...
1. **It narrows `meta`.** The manual version narrows `err.code` but leaves `err.meta` as the
   untyped union -- you'd still need a cast to read `retryAfter` off it. `hasErrorCode`'s return
   type narrows both.
